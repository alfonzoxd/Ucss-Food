...
<div class="relative bg-white border border-slate-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 shadow-sm overflow-hidden group">
    <div class="absolute top-0 right-0 w-32 h-32 bg-pink-100 rounded-full blur-3xl opacity-30 group-hover:opacity-50 transition-opacity"></div>
    <div class="relative z-10">
        <span class="inline-block px-3 py-1 bg-pink-100 text-pink-700 rounded-full text-xs font-bold mb-2 tracking-wide">POSTRES</span>
        <h2 class="text-2xl sm:text-3xl font-black text-slate-800">Endulza tu Día</h2>
        <p class="text-slate-600 font-medium text-sm sm:text-base mt-1">Deliciosos postres artesanales para el final perfecto.</p>
    </div>
</div>
...
<div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
    @forelse($postres as $postre)
        <div x-data="{ qty: 1 }"
            ...
            <div class="h-36 sm:h-44 md:h-48 bg-sky-50/50 p-4 sm:p-6 flex items-center justify-center relative overflow-hidden">
                ...
                <img src="{{ route('product.image', $postre['ref']) }}"
            ...
            <div class="p-3 sm:p-4 md:p-5 flex flex-col flex-1">
                ...
                <div class="mb-3 sm:mb-4">
                    <div class="flex justify-between items-start gap-2">
                        ...
                        <span class="font-black text-sky-700 text-sm sm:text-base md:text-lg whitespace-nowrap">
                            S/ {{ number_format($postre['price'], 2) }}
                        </span>
                    </div>
                    <p class="text-[10px] sm:text-xs text-slate-400 mt-1 font-medium">Dulce tentación</p>
                </div>
                ...
                <div class="mt-auto pt-3 sm:pt-4 border-t border-slate-50">
                    <div class="flex items-center justify-between gap-2 sm:gap-3">
                        ...
                        <button @click="addToCart({{ $postre['id'] }}, '{{ $postre['label'] }}', {{ $postre['price'] }}, qty); qty = 1"
    ...
    @empty
        <div class="col-span-full flex flex-col items-center justify-center py-8 sm:py-12 text-center opacity-60">
            ...
            <p class="text-sm sm:text-base text-slate-500 font-medium">No quedan postres hoy.</p>
        </div>
    @endforelse
</div>
